...
void redraw_tabs(uint64_t channel_id)
{
  ...
  for (tabpage_T *tp = first_tabpage; tp != NULL; tp = tp->tp_next) {
    ...
    PUT(tab_data, "selected", BOOL_OBJ(tp->tp_topframe == topframe));
...
void redraw_ruler(uint64_t channel_id, win_T *window, bool empty, char *relpos)
{
  ...
  PUT(event_data, "relpos", STRING_OBJ(relpos));
                             ...
                             Array *target)
{
  if (section_width) {
    ...
    PUT(section, "content", STRINGL_OBJ(ldata->buffer + ldata->offset,
                                        section_width));
