...
// This macro will call a function in the UI thread, waiting until the call
// is complete.
#define UI_WAIT_CALL(b, name, argc, ...)                                    \
  do {                                                                      \
...
{
  UI_WAIT_CALL(b, resize, 3, b, INT2PTR(width), INT2PTR(height));
}
...
{
  UI_WAIT_CALL(b, cursor_goto, 3, b, INT2PTR(row), INT2PTR(col));
}
...
{
  UI_WAIT_CALL(b, update_menu, 1, b);
}
...
{
  UI_WAIT_CALL(b, mode_change, 2, b, INT2PTR(mode));
}
...
static void ui_bridge_set_scroll_region(UI *b, int top, int bot, int left,
    int right)
{
  UI_WAIT_CALL(b, set_scroll_region, 5, b, INT2PTR(top), INT2PTR(bot),
...
{
  UI_WAIT_CALL(b, scroll, 2, b, INT2PTR(count));
}
...
{
  UI_WAIT_CALL(b, highlight_set, 2, b, &attrs);
}
...
{
  UI_WAIT_CALL(b, put, 3, b, text, INT2PTR(size));
}
...
{
  UI_WAIT_CALL(b, visual_bell, 1, b);
}
...
{
  UI_WAIT_CALL(b, update_fg, 2, b, INT2PTR(fg));
}
...
{
  UI_WAIT_CALL(b, update_bg, 2, b, INT2PTR(bg));
}
...
{
  UI_WAIT_CALL(b, set_title, 2, b, title);
}
...
{
  UI_WAIT_CALL(b, set_icon, 2, b, icon);
}
...
{
  UI_WAIT_CALL(b, set_encoding, 2, b, enc);
}
