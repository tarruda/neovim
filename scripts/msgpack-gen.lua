...
for i = 1, #api.functions do
  ...
  output:write('\n        return;')
  output:write('\n      }\n')
  -- Build validation/conversion code for each argument, also build a table
  -- with all
  for j = 1, #fn.parameters do
    local converted, convert_arg, param, arg
    local idx = tostring(j - 1)
    param = fn.parameters[j]
    arg = '(req->via.array.ptr[3].via.array.ptr + '..idx..')'
    converted = 'arg_'..tostring(j)
    convert_arg = 'msgpack_rpc_to_'..string.lower(param[1])
    output:write('\n      '..param[1]..' '..converted..';')
    output:write('\n      if (!'..convert_arg..'('..arg..', &'..converted..')) {')
    output:write('\n        msgpack_rpc_error("Wrong type for argument '..tostring(j)..', expecting '..param[1]..'", res);')
    output:write('\n        return;')
    output:write('\n      }\n')
  ...
  if fn.return_type == 'void' then
    ...
    local convert_result = 'msgpack_rpc_from_'..string.lower(fn.return_type)
    output:write('\n      '..convert_result..'(rv, res);\n')
  end
  output:write('\n      return;');
  output:write('\n    };');
