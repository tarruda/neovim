...
{
  return msgpack_rpc_to_uint64_t(obj, arg);
}
...
bool msgpack_rpc_to_object(msgpack_object *obj, Object *arg)
{
  switch (obj->type) {
    ...
    case MSGPACK_OBJECT_POSITIVE_INTEGER:
      arg->type = kObjectTypeUInt;
      return msgpack_rpc_to_uint64_t(obj, &arg->data.unsigned_integer);
...
bool msgpack_rpc_to_stringarray(msgpack_object *obj, StringArray *arg)
{
  ...
  arg->items = xmalloc(obj->via.array.size * sizeof(String));
...
bool msgpack_rpc_to_array(msgpack_object *obj, Array *arg)
{
  ...
  arg->size = obj->via.array.size;
  arg->items = xmalloc(obj->via.array.size * sizeof(Object));
...
bool msgpack_rpc_to_dictionary(msgpack_object *obj, Dictionary *arg)
{
  ...
  arg->size = obj->via.array.size;
  arg->items = xmalloc(obj->via.map.size * sizeof(KeyValuePair));
...
void msgpack_rpc_from_object(Object result, msgpack_packer *res)
{
  ...
  switch (result.type) {
    ...
    case kObjectTypeUInt:
      msgpack_rpc_from_uint64_t(result.data.unsigned_integer, res);
      break;
...
void msgpack_rpc_from_position(Position result, msgpack_packer *res)
{
  ...
  msgpack_pack_uint64(res, result.row);
  msgpack_pack_uint64(res, result.col);
}
...
void msgpack_rpc_from_dictionary(Dictionary result, msgpack_packer *res)
{
  ...
  for (size_t i = 0; i < result.size; i++) {
    msgpack_rpc_from_object(result.items[i].key, res);
