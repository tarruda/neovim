...
static void chld_handler(uv_signal_t *handle, int signum)
{
  ...
  for (int i = 0; i < MAX_RUNNING_JOBS; i++) {
    if ((job = table[i]) != NULL && job->opts.pty && job->pid == pid) {
      // We only care for processes that we created manually with forkpty
      // because libuv will handle those created with uv_spawn
      if (WIFEXITED(stat)) {
