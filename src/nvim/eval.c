...
static void f_jobstart(typval_T *argvars, typval_T *rettv)
{
  ...
  if (argvars[1].v_type == VAR_DICT) {
    ...
    common_job_callbacks(job_opts, &on_stdout, &on_stderr, &on_exit, &user);
    if (did_emsg) {
      return;
    }
  }
...
static inline JobOptions common_job_options(char **argv, ufunc_T *on_stdout,
    ufunc_T *on_stderr, ufunc_T *on_exit, typval_T user)
{
    ...
    ufunc_T **on_stderr, ufunc_T **on_exit, typval_T *user)
{
  ...
  *on_exit = get_dict_callback(vopts, "on_exit");
  ...
  dictitem_T *di = dict_find(vopts, (uint8_t *)"user", -1);
  if (di != NULL) {
    copy_tv(&di->di_tv, user);
  } else {
    user->v_type = VAR_NUMBER;
    user->v_lock = 0;
    user->vval.v_number = 0;
  }
}
...
static void on_job_event(Event event)
{
  ...
  if (argc > 3) {
    ...
    argv[3].v_lock = 0;
    argv[3].vval.v_string = (uint8_t *)ev->type;
  }
