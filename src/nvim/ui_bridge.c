...
#define UI(bridge) (((UIBridgeData *)bridge)->ui)
...
UI *ui_bridge_attach(UI *ui, ui_main_fn ui_main)
{
...
void ui_bridge_start(UIBridgeData *bridge, event_scheduler scheduler)
{
  uv_mutex_lock(&bridge->mutex);
  bridge->scheduler = scheduler;
  uv_cond_signal(&bridge->cond);
...
{
  UI_CALL(b, resize, 3, UI(b), INT2PTR(width), INT2PTR(height));
}
...
{
  UI_CALL(b, cursor_goto, 3, UI(b), INT2PTR(row), INT2PTR(col));
}
...
{
  UI_CALL(b, mode_change, 2, UI(b), INT2PTR(mode));
}
...
static void ui_bridge_set_scroll_region(UI *b, int top, int bot, int left,
    int right)
{
  UI_CALL(b, set_scroll_region, 5, UI(b), INT2PTR(top), INT2PTR(bot),
...
{
  UI_CALL(b, scroll, 2, UI(b), INT2PTR(count));
}
...
static void ui_bridge_highlight_set(UI *b, HlAttrs attrs)
{
  ...
  *a = attrs;
  UI_CALL(b, highlight_set, 2, UI(b), a);
}
...
{
  uint8_t *t = xmalloc(size);
  memcpy(t, text, size);
  UI_CALL(b, put, 3, UI(b), t, INT2PTR(size));
}
...
{
  UI_CALL(b, update_fg, 2, UI(b), INT2PTR(fg));
}
...
{
  UI_CALL(b, update_bg, 2, UI(b), INT2PTR(bg));
}
...
{
  UI_CALL(b, set_title, 2, UI(b), title ? xstrdup(title) : NULL);
}
...
{
  UI_CALL(b, set_icon, 2, UI(b), icon ? xstrdup(icon) : NULL);
}
...
{
  UI_CALL(b, set_encoding, 2, UI(b), xstrdup(enc));
}
