...
{
  paramp->input_isatty = isatty(fileno(stdin));
  paramp->output_isatty = isatty(fileno(stdout));
  paramp->err_isatty = isatty(fileno(stderr));
...
static void usage(void)
{
  ...
  mch_msg(_("  --headless            Don't start an user interface"));
  mch_msg(_("  --embed               Use stdin/stdout as a msgpack-rpc channel. "
                                     "This can be used for embedding Neovim "
                                     "into other programs. Implies --headless"));
