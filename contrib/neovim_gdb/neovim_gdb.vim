...
let s:run_gdb = "gdb -q build/bin/nvim"
let s:breakpoints = {}
...
function! s:GdbSpawn(gdbserver_cmd, gdbclient_cmd)
  " Create a new tab
  tabnew
  let t:match_buffer = []
  ...
  sp | vsp
  " spawn gdbserver 
  enew | let t:gdbserver_id = termopen(a:gdbserver_cmd)
  ...
  let t:handlers = [
        ...
        \ ['\v^.{-}\ +at\ +(.{-}):\d+', function('s:JumpToFile')],
        \ ['\v^(\d+)\s+', function('s:JumpToLine')],
...
function! s:GdbNvim()
  call s:GdbSpawn(printf(s:run_nvim, s:gdb_port), s:run_gdb)
endfunction


function! s:GdbTest(...)
  let cmd = printf(s:run_tests, s:gdb_port)
  if a:0
    let cmd = 'TEST_SCREEN_TIMEOUT=1000000 TEST_TAG='.a:1.' '.cmd
  endif
  call s:GdbSpawn(cmd, s:run_gdb)
...
function! s:Attach()
  call s:GdbSend(printf('target remote localhost:%d', s:gdb_port))
endfunction
...
function! s:JumpToLine(line, ...)
  ...
  if exists('t:pending_file')
    let target_buf = bufnr(t:pending_file, 1)
    if bufnr('%') != target_buf
      exe 'buffer ' target_buf
      let t:current_buf = target_buf
    endif
    unlet t:pending_file
  endif
  exe ':' a:line
...
function! s:JumpToFile(file, ...)
  let t:pending_file = a:file
endfunction
...
command! GdbDebugNvim call s:GdbNvim()
