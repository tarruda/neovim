...
static void push_keys_no_timeout(TermInput *input)
{
  do {
    char *ptr = rbuffer_read_ptr(input->read_buffer);
    if (*ptr == '\x1b') {
      termkey_push_bytes(input->tk, ptr, 1);
      rbuffer_consumed(input->read_buffer, 1);
      tk_getkeys(input, true);
      continue;
    }
...
static void read_cb(RStream *rstream, void *rstream_data, bool eof)
{
  ...
  if (get_key_code_timeout() <= 0) {
    // libtermkey doesn't have a timeout option for interpreting escape, so we
    // have to do it ourselves by separating the escape bytes from the rest of
    // the input(only do this when ttimeoutlen <= 0)
    push_keys_no_timeout(rstream_data);
    return;
  }

  do {
    size_t consumed = termkey_push_bytes(input->tk,
        rbuffer_read_ptr(input->read_buffer),
        rbuffer_pending(input->read_buffer));
    rbuffer_consumed(input->read_buffer, consumed);
    tk_getkeys(input, false);
  } while (rbuffer_pending(input->read_buffer));
}
...
static TermInput *term_input_new(void)
{
  ...
  rv->in_fd = isatty(0) ? 0 : (isatty(2) ? 2 : 0);
