...
function! s:Gdb.attach()
  call self.send(printf('target remote localhost:%d', self._port))
endfunction
...
function! s:Spawn(server_cmd, client_cmd, port, reattach)
  if exists('g:gdb')
    throw 'Gdb already running'
  endif
  let gdb = g:Expect.create(s:GdbRunning, copy(s:Gdb))
  " gdbserver port
  let gdb._port = a:port
  let gdb._reattach = a:reattach
  ...
  if type(a:server_cmd) == type('')
    ...
    vsp | enew | let gdb._server_id = termopen(printf(a:server_cmd, a:port))
...
function! s:Test(...)
  ...
  call s:Spawn(cmd, s:run_gdb, s:gdb_port, 1)
endfunction
...
command! GdbDebugNvim call s:Spawn(s:run_nvim, s:run_gdb, s:gdb_port, 0)
command! -nargs=1 GdbDebugServer call s:Spawn(0, s:run_gdb, <f-args>, 0)
command! -nargs=? GdbDebugTest call s:Test(<f-args>)
