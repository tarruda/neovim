...
local rbuffer = helpers.cimport("./test/unit/helpers/rbuffer.h")
...
describe('rbuffer functions', function()
  ...
  local function write(str)
    ...
    return rbuffer.rbuffer_write(rbuf, buf, len)
  end

  local function read(len)
    local buf = c_char(len)
  ...
  describe('rbuffer_cmp', function()
    local function cmp(str)
      local len = string.len(str)
      local rv = rbuffer.rbuffer_cmp(rbuf, c_char(len, str), len)
      if rv == 0 then
        return 0
      else
