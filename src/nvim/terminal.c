...
Terminal *terminal_new(uint16_t width, uint16_t height, bool force)
{
  ...
  for (int col = 0; col < width; col++) {
    ...
    if (hl_attrs || vt_fg != -1 || vt_bg != -1) {
      attr_id = get_attr_entry(&(attrentry_T) {
        ...
        // TODO(tarruda): let the user customize the rgb color palette
...
static void on_resize(Event event)
{
  ...
  if (!term->altscreen && term->new_height < height) {
    term->buf->b_p_ma = true; term->buf->b_p_ro = false;
    buf_T *save_curbuf = NULL;
    win_T *save_curwin = NULL;
    tabpage_T *save_curtab = NULL;
  ...
  vterm_screen_flush_damage(term->vts);
  term->resize_cb(term->resize_data, (uint16_t)width, (uint16_t)height);
}
